...
public class RowStoreTest {
    public static void main(String[] args) throws IOException {
        ...
        for (int run = 1; run <= 5; run++) {
            ...
            ColumnStore columnStore = CSVLoader.loadCSV(filePath);
            long csvLoadTime = (System.nanoTime() - startLoadTime) / 1_000_000;
            System.out.println("CSV Loading Time: " + csvLoadTime + " ms");
            
            // Convert and load data into RowStore
            System.out.println("Converting and loading data into RowStore...");
            long startConvertTime = System.nanoTime();
            List<Map<String, Object>> rows = new ArrayList<>();
            String firstColumn = columnStore.getColumnNames().iterator().next();
            for (int i = 0; i < columnStore.getColumn(firstColumn).size(); i++) {
                Map<String, Object> row = new HashMap<>();
                for (String key : columnStore.getColumnNames()) {
                    row.put(key, columnStore.getColumn(key).get(i));
                }
                rows.add(row);
            }
            rowStore.addRows(rows);
            long convertTime = (System.nanoTime() - startConvertTime) / 1_000_000;
            System.out.println("Data Conversion and Loading Time: " + convertTime + " ms");
            ...
            System.out.println("Minimum Price Total Time (including loading): " + (csvLoadTime + convertTime + minPriceTime) + " ms");
            ...
            System.out.println("Standard Deviation Total Time (including loading): " + (csvLoadTime + convertTime + stdDevTime) + " ms");
            ...
            System.out.println("Average Price Total Time (including loading): " + (csvLoadTime + convertTime + avgPriceTime) + " ms");
            ...
            System.out.println("Price per Square Meter Total Time (including loading): " + (csvLoadTime + convertTime + pricePerSqmTime) + " ms");
